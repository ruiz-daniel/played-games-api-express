...
const getGameFields =
  "id, name, release_dates.platform.name, release_dates.human, release_dates.y, release_dates.m, summary, involved_companies.company.name, involved_companies.developer, involved_companies.publisher, cover.url, url, genres.name";
...
module.exports.handler = {
  ...
  async getByName(name, token, limit = 30) {
    ...
    const response = await axios.request({
      method: "post",
      url: `${igdbUrl}/games`,
      data: `fields ${getGameFields}; where name ~ "${parsedName}" & parent_game = null; limit ${limit};`,
